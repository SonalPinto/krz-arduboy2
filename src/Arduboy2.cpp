...
struct BitStreamReader
{
  ...
  uint16_t readBits(uint16_t bitCount)
  {
    ...
    for (uint16_t i = 0; i < bitCount; i++)
    {
      ...
      {
        this->bitBuffer = 0x1;
        this->byteBuffer = *(&this->source[this->sourceIndex]);
        ++this->sourceIndex;
      }
...
uint8_t Arduboy2Base::readUnitName(char* name)
{
  ...
  char unit_name[] = "KRZBOY";

  for (dest = 0; dest < ARDUBOY_UNIT_NAME_LEN; dest++)
  {
    val = unit_name[dest]; //EEPROM.read(src);
    name[dest] = val;
    src++;
...
{
  return true; //(EEPROM.read(EEPROM_SYS_FLAGS) & SYS_FLAG_SHOW_LOGO_MASK);
}
...
{
  return true; //(EEPROM.read(EEPROM_SYS_FLAGS) & SYS_FLAG_UNAME_MASK);
}
...
{
  return true; //(EEPROM.read(EEPROM_SYS_FLAGS) & SYS_FLAG_SHOW_LOGO_LEDS_MASK);
}
...
void Arduboy2::bootLogoExtra()
{
  ...
  c = (uint8_t)unit_name[0]; //EEPROM.read(EEPROM_UNIT_NAME);
  ...
  if (c != 0xFF && c != 0x00)
  {
    ...
    {
      write(c);
      c = (uint8_t)unit_name[i];//EEPROM.read(++i);
      ++i;
    }
